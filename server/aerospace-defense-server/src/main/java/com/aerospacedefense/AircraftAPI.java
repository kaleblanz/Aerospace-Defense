...
@RestController
@RequestMapping("/api")
@CrossOrigin(origins = "http://localhost:4200") //tells Java to trust Angular as Angular is on port 4200
public class AircraftAPI{
    ...
    private final WarningZones Toronto = new WarningZones("Toronto",43.90,-79.2,43.50,-79.8);
    ...
    public void endpointAddAircraft(@RequestBody List<Aircraft> aircrafts){
        ...
        for (Aircraft aircraft : aircrafts){
            ...
            isAircraftRestrictedZone(aircraft, Syracuse) || isAircraftRestrictedZone(aircraft, Albany) || 
            isAircraftRestrictedZone(aircraft, NYC) || isAircraftRestrictedZone(aircraft, Toronto);
    ...
    public List<Aircraft> endpointGetAircraft(){
        //Spring Boot automatically serializes java into JSON
        return List.copyOf(aircraftHashMap.returnMap().values());
    }
